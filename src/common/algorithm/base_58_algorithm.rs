...
pub struct Base58Algorithm;
impl Base58Algorithm {
    ...
    pub fn encode(input: &[u8]) -> String {
        ...
        while input_start < input.len() {
            // 执行除法取模操作并获取编码字符
            let remainder = Self::divmod(&mut input, input_start, 256, 58);
            output_start -= 1;
    ...
    pub fn decode(input: &str) -> Result<Vec<u8>, &'static str> {
        ...
        while input_start < input58.len() {
            // 执行除法取模操作并获取解码字节
            let remainder = Self::divmod(&mut input58, input_start, 58, 256);
            output_start -= 1;
    ...
    /// 将 Base58 编码字符串解码为 BigUint
    pub fn decode_to_big_integer(input: &str) -> Result<BigUint, &'static str> {
